...
export function DosaGrillsColumn() {
  ...
  // Partition derived in render — lists are small (< 20 items); no useMemo needed
...
// Memoized section — only re-renders when items array changes
...
// Memoized per item — stable as long as name/qty unchanged
const AggregatedItem = memo(function AggregatedItem({ item }: { item: DosaGrillAggregatedItem }) {
  return (
...
const ColumnHeader = memo(function ColumnHeader({ count }: { count: number }) {
  return (
